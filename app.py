...
class CupMatch(db.Model):
    ...
    # State: waiting_for_ready -> waiting_for_code -> in_progress -> finished
    ...
    def get_picked(self):
        try: return json.loads(self.picked_maps) if self.picked_maps else []
        except: return []
    def get_scores_a(self):
        try: return json.loads(self.scores_a) if self.scores_a else []
        except: return []
    def get_scores_b(self):
        try: return json.loads(self.scores_b) if self.scores_b else []
        except: return []
...
def handle_scoring_logic(match, form_data, user):
    # Logik für normale Turniere/Ligen (nicht Cup)
...
def login():
    if request.method == 'POST':
        ...
        if clan and check_password_hash(clan.password, password):
            session['clan_id'] = clan.id; flash(f'Willkommen {clan.name}!', 'success'); return redirect(url_for('clan_dashboard'))
...
def dashboard():
    ...
    maps = Map.query.filter_by(is_archived=False).all() # Nur Aktive Maps
...
def clan_create_team():
    ...
    if not User.query.filter_by(username=name).first():
        db.session.add(User(username=name, token=str(random.randint(10000, 99999)), clan_id=c.id)); db.session.commit(); flash(f'Team {name} erstellt!', 'success')
    return redirect(url_for('clan_dashboard'))
...
def create_admin():
    if not current_user.is_admin: return redirect(url_for('dashboard'))
    db.session.add(User(username=request.form.get('username'), password=generate_password_hash(request.form.get('password'), method='pbkdf2:sha256'), is_admin=True)); db.session.commit()
...
def create_mod():
    if not current_user.is_admin: return redirect(url_for('dashboard'))
    db.session.add(User(username=request.form.get('username'), password=generate_password_hash(request.form.get('password'), method='pbkdf2:sha256'), is_mod=True)); db.session.commit()
    return redirect(url_for('users_manager'))
...
def create_user():
    ...
    name = f"{Clan.query.get(c_id).name}.{request.form.get('username')}" if c_id else request.form.get('username')
    db.session.add(User(username=name, token=str(random.randint(10000, 99999)), clan_id=c_id)); db.session.commit()
    return redirect(url_for('users_manager'))
...
def admin_reset_clan_password(clan_id):
    ...
    if request.form.get('new_password'):
        Clan.query.get_or_404(clan_id).password = generate_password_hash(request.form.get('new_password'), method='pbkdf2:sha256'); db.session.commit()
    return redirect(url_for('users_manager'))
...
def create_tournament():
    ...
    if request.method == 'POST':
        ...
        for i in range(0, len(selected), 2): db.session.add(Match(tournament_id=t.id, team_a=selected[i], team_b=selected[i+1], state='ban_1_a', round_number=1, match_index=i//2))
        db.session.commit()
        prev = [m for m in t.matches if m.round_number==1]
        for r in range(2, int(math.ceil(math.log2(len(selected))))+1):
            curr = []
...
def cup_match_view(match_id):
    ...
    if request.method == 'POST':
        ...
        if 'set_maps' in request.form and (current_user.is_admin or current_user.is_mod):
            ...
            try:
                # KORREKTUR: 1-basierter Index für Inputs (Map 1, Map 2, Map 3)
...
def match_chat_api(match_id):
    try:
        match = Match.query.get_or_404(match_id)
        if request.method=='POST' and request.json.get('message'): db.session.add(ChatMessage(match_id=match.id, username=current_user.username, message=request.json['message'], is_admin=current_user.is_admin, is_mod=current_user.is_mod)); db.session.commit(); return json.dumps({'status':'ok'})
...
def lobby_code_api(match_id):
    match = Match.query.get_or_404(match_id)
    if request.method == 'GET': return json.dumps({'lobby_code': match.lobby_code or ''})
    if current_user.is_admin or current_user.is_mod or current_user.username in [match.team_a, match.team_b]:
        match.lobby_code = request.json.get('lobby_code', '').strip(); db.session.commit(); return json.dumps({'status': 'ok'})
...
def cup_chat_api(match_id):
    try:
        match = CupMatch.query.get_or_404(match_id)
        if request.method=='POST' and request.json.get('message'): db.session.add(CupChatMessage(cup_match_id=match.id, username=current_user.username, message=request.json['message'], is_admin=current_user.is_admin, is_mod=current_user.is_mod)); db.session.commit(); return json.dumps({'status':'ok'})
...
def league_match_chat_api(match_id):
    try:
        match = LeagueMatch.query.get_or_404(match_id)
        if request.method=='POST' and request.json.get('message'): db.session.add(LeagueChatMessage(league_match_id=match.id, username=current_user.username, message=request.json['message'], is_admin=current_user.is_admin, is_mod=current_user.is_mod)); db.session.commit(); return json.dumps({'status':'ok'})
...
def league_match_state(match_id):
    ...
    return json.dumps({'state':m.state,'active_team':m.team_a if m.state.endswith('_a') else (m.team_b if m.state.endswith('_b') else None),'banned':m.get_banned(),'picked':m.get_picked(),'lobby_code':m.lobby_code,'confirmed_a':m.confirmed_a,'confirmed_b':m.confirmed_b})

@app.route('/api/match/<int:match_id>/state')
@login_required
def match_state_api(match_id):
    m = Match.query.get_or_404(match_id)
    return json.dumps({'state':m.state,'active_team':m.team_a if m.state.endswith('_a') else (m.team_b if m.state.endswith('_b') else None),'banned':m.get_banned(),'picked':m.get_picked(),'lobby_code':m.lobby_code})
