...
def calculate_map_wins(scores_a, scores_b):
    wins_a = 0
    wins_b = 0
    # Gehe durch alle gespielten Maps und zähle wer gewonnen hat
...
class CupMatch(db.Model):
    ...
    def get_map_wins(self):
        # Aktualisiert auf die neue globale Funktion
        return calculate_map_wins(self.get_scores_a(), self.get_scores_b())
...
def handle_pick_ban_logic(match, selected_map):
    ...
    if selected_map in current_banned or selected_map in current_picked: return False, "Vergeben."
...
def advance_winner(match):
    ...
    # NEU: Berechnung nach Map Wins
    wa, wb = match.get_map_wins()
    if wa > wb: win = match.team_a
    elif wb > wa: win = match.team_b
    else: win = match.team_a if match.total_score_a > match.total_score_b else match.team_b # Fallback Total Score
...
def handle_scoring_logic(match, form_data, user):
    ...
    if user.is_admin or user.is_mod:
        ...
        # Bei Liga auch Lineup direkt speichern wenn angegeben (Admin Override)
        if isinstance(match, LeagueMatch) and lineup_list: 
            pass 
...
def login():
    if request.method == 'POST':
        ...
        if user:
            if (user.is_admin or user.is_mod) and check_password_hash(user.password, password): login_user(user); return redirect(url_for('dashboard'))
            elif user.token == password: login_user(user); return redirect(url_for('dashboard'))
        clan = Clan.query.filter_by(name=username).first()
        if clan and check_password_hash(clan.password, password): session['clan_id'] = clan.id; return redirect(url_for('clan_dashboard'))
        flash('Fehler.', 'error')
    return render_template('login.html')

@app.route('/logout')
def logout(): logout_user(); session.pop('clan_id', None); return redirect(url_for('login'))
...
def dashboard():
    ...
    # Filterung der aktiven und archivierten Elemente
    return render_template('dashboard.html', 
                           ...
                           maps=Map.query.filter_by(is_archived=False).all(), # Nur aktive Maps im Dashboard
    ...
    if request.method == 'POST':
        ...
        for r in range(num_rounds):
            for i in range(len(teams)//2):
                t1, t2 = teams[i], teams[len(teams)-1-i]
                if t1 and t2: db.session.add(LeagueMatch(league_id=l.id, team_a=t1, team_b=t2, round_number=r+1))
...
def league_match_view(match_id):
    ...
    if request.method == 'POST':
        ...
        # 3. CONFIRMATION (Phase 2)
            ...
            # Wenn BEIDE bestätigt haben -> Finish
            if match.confirmed_a and match.confirmed_b:
                match.state = 'finished'
                # Jetzt die Draft Lineups in die echte Spalte schieben
...
# --- RESTLICHE ROUTEN (TURNIER, CUP etc) ---
